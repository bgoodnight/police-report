...
```{r echo=FALSE, fig.height=1.7, fig.width=8}
...
**`r sum(data$gsl_arrest, na.rm=TRUE)`** of the **`r nrow(data)`** officers in `r state.name[grep(state, state.abb)]` responded correctly when asked whether `r state.name[grep(state, state.abb)]` provides immunity from arrest. This accounted for **`r round(sum(data$gsl_arrest, na.rm=TRUE)/nrow(data)*100,2)`%** of all participating officers.

**`r sum(data$gsl_charge, na.rm=TRUE)`** officers responded correctly when asked whether `r state.name[grep(state, state.abb)]` provides immunity from criminal charges. This accounted for **`r round(sum(data$gsl_charge, na.rm=TRUE)/nrow(data)*100,2)`%** of all participating officers in `r state.name[grep(state, state.abb)]`.
